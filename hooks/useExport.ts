  ...
  export default () => {
    ...
    const getPresentationNameFromUrl = (): string => {
      ...
      setTimeout(() => {
        ...
        toImage(domRef, config)
          .then((dataUrl) => {
            ...
            saveAs(dataUrl, `${getPresentationNameFromUrl()}.${format}`)
          })
          .catch(() => {
            exporting.value = false
        ...
        for (const item of obj) {
          ...
          if ('tagName' in item && item.tagName === 'br') {
            ...
            if (styleObj['font-size']) {
              options.fontSize = parseInt(styleObj['font-size'])
            }
            if (styleObj['color']) {
    ...
    const exportPPTX = (_slides: Slide[], masterOverwrite: boolean, ignoreMedia: boolean) => {
        ...
        for (const el of slide.elements) {
          if (el.type === 'text') {
            ...
            if (el.text) {
              ...
              const textProps = formatHTML(el.text.content) // This contains parsed font sizes
              if (el.fill === 'none' || el.fill === ' ')
              {
                const options: pptxgen.TextPropsOptions = {
                  x: el.left / INCH_PX_RATIO,
                  y: el.top / INCH_PX_RATIO,
            ...
            if ((el.data.series.length > 0 && el.chartType !== 'scatter') || el.chartType === 'pie' || el.chartType === 'ring') {
            ...
            else if (el.chartType === 'horizontalBar') {
              type = pptx.ChartType.bar
              options.barDir = 'bar'
              if (el.options?.stack) options.barGrouping = 'stacked'
            }
      ...
      setTimeout(() => {
        pptx
          .writeFile({ fileName: `${getPresentationNameFromUrl()}.pptx` })
    ...
    const savePPTX = async (_slides: Slide[], masterOverwrite: boolean, ignoreMedia: boolean) => {
      exporting.value = true
      const pptx = new pptxgen()
